...
export async function POST(req:Request){
  ...
  try {
    ...
    const lines = items.map(it=>{
      ...
      if (!db) throw new Error('Item not found')
    ...
    if (coupon_code) {
      ...
      if (cp[0]) {
        discount = cp[0].discount_type==='percent' ? subtotal * Number(cp[0].value)/100 : Number(cp[0].value)
      }
    }
    ...
    for (const l of lines) {
      await client.query(
        `insert into order_items (order_id, item_id, qty, sale_price, cost_price, line_total, line_profit)
         values ($1,$2,$3,$4,$5,$6,$7)`,
        [orderId, l.item_id, l.qty, l.sale_price, l.cost_price, l.line_total, l.line_profit]
      )
    }
    ...
    const msg = `${header}\n${linesText}\n———\nTạm tính: ${subtotal.toLocaleString()}₫\nGiảm: ${discount.toLocaleString()}₫\nTổng: ${total.toLocaleString()}₫\nLợi nhuận: ${profit.toLocaleString()}₫\n#${orderId}`
    await sendTelegram(msg)

    return new Response(JSON.stringify({ id: orderId, subtotal, discount, total, profit, created_at: o[0].created_at }), { headers: { 'Content-Type':'application/json' } })
  } catch (e:any) {
