...
export async function POST(req:Request){ if (!await requireAdmin()) return new Response('Unauthorized',{status:401}); await ensureSchema(); const b = await req.json(); const { rows } = await pool.query(`insert into coupons(code, percent_off, max_uses, valid_from, valid_to, active) values ($1,$2,$3,$4,$5, coalesce($6,true)) returning *`, [b.code, Number(b.percent_off)||0, b.max_uses?Number(b.max_uses):null, b.valid_from||null, b.valid_to||null, b.active]); return Response.json(rows[0]) }
