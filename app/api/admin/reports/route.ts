...
export async function GET(req:Request){
  if (!await requireAdmin()) return new Response('Unauthorized',{status:401}); await ensureSchema()
  const { searchParams } = new URL(req.url)
  const r = (searchParams.get('range')||'day') as 'day'|'week'|'month'|'year'
  const meta = (RANGE as any)[r] || RANGE.day
  const sql = `select date_trunc($1, created_at) as bucket, sum(total)::float as revenue, sum(profit)::float as profit, count(*)::int as orders from orders where created_at >= now() - interval '${meta.interval}' group by 1 order by bucket asc`
