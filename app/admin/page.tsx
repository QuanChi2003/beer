...
const f = (u:string)=>fetch(u).then(r=>{ if (r.status===401) throw new Error('unauth'); return r.json() })
      ...
      {!data ? <p>Đang tải…</p> : (
        <div className="mt-3 overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>{data.map((r:any)=>(<tr key={r.bucket}><td>{r.bucket}</td><td>{Number(r.revenue).toLocaleString()}₫</td><td>{Number(r.profit).toLocaleString()}₫</td><td>{r.orders}</td></tr>))}</tbody>
          </table>
        </div>
      )}
    </div>
...
function ManageCategories(){
  ...
  const submit = async()=>{
    const res = await fetch('/api/admin/categories',{method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({name, parent_id: parent||null, pos:Number(pos)})})
    if (res.ok){ setName(''); setParent(''); setPos(0); mutate() } else { alert('Chế độ No-DB không cho tạo/sửa danh mục'); }
  }
  ...
  return (
    <div className="card p-4">
      <h2 className="font-semibold mb-2">Phân loại (có thể tạo danh mục & nhóm con khi có DB)</h2>
...
function ManageItems(){
  ...
  const submit = async()=>{
    ...
    if (res.ok){ setForm({name:'', sale_price:0, cost_price:0, category_id:'', image_url:'', description:''}); mutate() } else { alert('Chế độ No-DB không cho thêm món'); }
  }
  return (
    <div className="card p-4">
        ...
        <div className="sm:col-span-2 overflow-x-auto">
          {!data ? <p>Đang tải…</p> : (
            <table className="w-full text-sm">
              ...
              <tbody>
                {data.map((it:any)=>(
                  <tr key={it.id}><td>{it.name}</td><td>{it.category_id?.slice(0,6)||it.category_id}</td><td>{Number(it.sale_price).toLocaleString()}₫</td><td>{Number(it.cost_price).toLocaleString()}₫</td><td>{it.is_active!==false?'Hiện':'Ẩn'}</td></tr>
                ))}
              </tbody>
            </table>
          )}
...
function OrdersHint(){
  return (
    <div className="card p-4">
      ...
      <p className="text-sm text-neutral-600">Đơn hàng tạo ở Trang Thanh toán. Ở chế độ No-DB, hệ thống vẫn gửi Telegram nhưng không lưu đơn. Thêm DB để xem báo cáo chi tiết.</p>
    </div>
  )
}
