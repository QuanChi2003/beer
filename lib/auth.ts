...
export async function requireAdmin(){ const token=cookies().get('admin_session')?.value; if(!token) return null; try{ const {payload}=await jwtVerify(token, secret); if((payload as any).role==='admin') return payload }catch{}; return null }
