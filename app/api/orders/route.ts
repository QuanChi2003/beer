...
export async function POST(req:Request){
  ...
  if (coupon_code){
    ...
    const c = cp.rows[0]; if(c){ discount = Math.floor(Number(subtotal) * Number(c.percent_off) / 100) }
  }
  ...
  try{
    ...
    if (phone){
      ...
      if (existing.rowCount){ const newPts = Number(existing.rows[0].points)+pt; const tier = calcTier(newPts); await pool.query('update members set points=$1, tier=$2, updated_at=now() where phone=$3',[newPts, tier, phone]) }
